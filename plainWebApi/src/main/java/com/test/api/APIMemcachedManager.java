...
public class APIMemcachedManager {
	...
	private Logger logger = LoggerFactory.getLogger(APIMemcachedManager.class);
	private static final String SEPARATOR = "-";

	private static APIMemcachedManager instance;
	private MemcachedClient client;
	private String channel;
	private String addresses;
	public static String offerUrl = "http://www.51offer.com";

	private APIMemcachedManager() {
		logger.info("initClient memcached");
		Properties config = new Properties();
		InputStream input = null;
		ClassLoader cl = Thread.currentThread().getContextClassLoader();
		try {
			input = cl.getResourceAsStream("api.properties");
	...
	/**
	 * 单例 spy MemcachedClient 是一种nio方式的连接，为避免原则性操作问题，整个应用只允许打开一个连接
	 * 
	 * @return
	 */
	public static APIMemcachedManager getInstance() {
		if (null == instance) {
			instance = new APIMemcachedManager();
		}
		return instance;
	}
	...
	public Object get(String key) {
		...
		// logger.debug("get value from memcache key:{} value:{}", realKey,
		// value);
	...
	public void set(String key, int expre, Object value) throws IOException {
		...
		String realKey = composeKey(key);
		// logger.debug("begin to set value to memcache key:{} value:{}",
		// realKey, value);
		logger.debug("begin to set value to memcache key:{}", realKey);
		client.set(realKey, expre, value);
		// logger.debug("success set value to memcache key:{} value:{}",
		// realKey, value);
	}

	/**
	 * 一个key只允许包含一条记录 不等待结果集返回 内部串行实现
	 * 
	 * @param key
	 * @param expre
	 *            秒
	 ...
	 */
	public boolean addWait(String key, int expre, Object value) throws Exception {
		Assert.notNull(key, "设置memcache的key不可为空");
		Assert.isTrue(expre >= 0, "expre time must >= 0");
		String realKey = composeKey(key);
		// logger.debug("begin to set value to memcache key:{} value:{}",
